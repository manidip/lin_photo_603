...
<html lang="en">
   <?php
    ...
    if(isset($_GET['action']) && $_GET['action']=='delete')
    {
      ...
      $sql="DELETE FROM `latest_blog_posts` WHERE `post_id`='$postid'";
      $query=mysql_query($sql);   
    }
    ?> 
<head>
...
<body >
  ...
  <div  class="menu" style="height: 10px; float: right;">
    ...
    <div  class="selection" style="height:2000px; margin-top: 8px; float: left; width:230px;background: -moz-linear-gradient(top, #1e5799 0%, #2989d8 50%, #207cca 51%, #7db9e8 100%);">
      ...
      <ul>
        ...
        <div id=mpostlist><ul>
              <li><a href="#" style="color: aqua;">View Post</a></li>
              <li><a href="createpost.php">Create Post</a></li>
        
            </ul>
        </div>
    </div>
    <div style="height:2000px; margin-top: 8px;margin-left: 205px; width:835px;background: -moz-linear-gradient(top, #00b7ea 0%, #009ec3 100%);">
    <br>
    <div class="form-style-5">
<form action="insertcat.php" method="post">
<fieldset>
<table style="border: 2px;">
        <tr><th>Title<th>Contant<th>Image<th>Date<th colspan=2>Action</th></tr>
    <?php
    $sql="SELECT * FROM `latest_blog_posts` ORDER BY `latest_blog_posts`.`post_id` DESC ";
    $query=mysql_query($sql);
    while($row=mysql_fetch_array($query))
    {
      $contant=$row[post_contant];
      $length=strlen($row[post_contant]);
      
  echo "<tr><td>$row[post_headline]<td>";
  for($i=0;$i<$length;$i++)
                     {
                        if($i<200)
                        {
  ...
  echo"<td><center><img src='../$row[post_photo_url]' height=25px width=25px/></a></center><td>$row[post_date]<td><center><a href=viewpost.php?action=delete&pid=$row[post_id]><img src=delete.png height=25px width=25px/></a></center><td><center><a href=createpost.php?action=edit&pid=$row[post_id]><img src=edit.png height=25px width=25px/></a></center></tr>";
    }
?>
    </table>
</fieldset>
